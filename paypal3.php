...
if(isset($token))
{
    ...
    $info['user_id']    = (empty($_SESSION['user']['id'])                                    ? ' ' : $_SESSION['user']['id'] );
    ...
    $info['subtotal']   = (empty($obj->transactions[0]->amount->details->subtotal)           ? ' ' : $obj->transactions[0]->amount->details->subtotal);
    $info['cart_id']    = (empty($obj->cart)                                                 ? ' ' : $obj->cart);
    $info['phone']      = (empty($obj->payer->payer_info->phone)                             ? ' ' : $obj->payer->payer_info->phone);
    ...
    $execution->setPayerId($info['payment_id']);
    $transaction = new Transaction();
    $amount      = new Amount();
    $details     = new Details();

    if(in_array(strtolower($info['state']) ,array("CCalifornia", "CCA", "CCa", "Cca", "cCalifornia" )))
    {
        $tax        = ($info['subtotal'] * $taxRate) / 100;
        $tax        = number_format($tax, 2 );
    }

    $details->setShipping($info['shipping'])
        ->setTax($tax)
        ->setSubtotal($info['subtotal']);

    $final = $info['shipping'] + $info['subtotal'] + $tax;

    $amount->setCurrency('USD');
    $amount->setTotal($final);
    ...
    try {
        $result   = $all->execute($execution, $apiContext);
        $done     = json_decode($result);
        $status   = ($done->state == 'approved'  ? 'succeeded' : $done->state );
      ...
      fname, lname, email, country, address1, address2, city, state, zipcode, same_as, billing_fname, billing_lname, billing_country, billing_address1, billing_city, billing_state, billing_zipcode, phone, card_number, cvv, month, year, package_id, shipping_id ,token, qty, note ,added_date, avs, product_type, device, domain_url, tax
      )value(
      :fname, :lname, :email, :country, :address1, :address2, :city, :state, :zipcode,:same_as, :billing_fname, :billing_lname, :billing_country, :billing_address1, :billing_city, :billing_state,:billing_zipcode,  :phone, :card_number, :cvv, :month, :year, :package_id, :shipping_id ,:token, :qty, :note ,:added_date, :avs, :product_type, :device, :domain_url, :tax
      )';
        ...
        $logFile = '/home/customer/www/cutiebeauti.com/public_html/alllog-log';
        ...
        //    watch($done->state);
        //    echo "Executed Payment". $all->getId();
    } catch (Exception $ex) {
        //   echo $ex;
    }
